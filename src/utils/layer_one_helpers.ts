...
export function ardriveTxFilter(edge: GQLEdgeInterface) {
	const appName = edge.node.tags.find((tag) => tag.name === APP_NAME_TAG)?.name;
...
export function decodeTags(encodedTags: GQLTagInterface[]): GQLTagInterface[] {
	const tags = encodedTags.map((tag) => {
		try {
			return {
				name: fromB64Url(tag.name).toString().toLowerCase(),
				value: fromB64Url(tag.name).toString()
			};
		} catch (error) {
...
export function getTagValue(tags: GQLTagInterface[], name: string): string | undefined {
	const contentTypeTag = tags.find((tag) => {
		try {
			return fromB64Url(tag.name).toString().toLowerCase() == name.toLowerCase();
		} catch (error) {
			return undefined;
		}
	});
	try {
		return contentTypeTag ? fromB64Url(contentTypeTag.value).toString() : undefined;
	} catch (error) {
		return undefined;
	}
}
